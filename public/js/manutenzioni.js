...
async function loadMan(){
  ...
  el.innerHTML=data.map(m=>{
    ...
    const scadColor=(!isNaN(gg))?(gg<0?'var(--red)':gg<30?'var(--orange)':gg<90?'var(--gold)':'var(--green)'):'var(--muted)';
    return`<div class="int-card" style="border-left:3px solid ${PRIOR_COLORS[m.priorita]||'var(--border)'};">
      <div class="int-card-hdr">
        ...
        <strong style="font-size:14px;color:#fff;">${esc(m.tipo)}</strong>
        ...
        <span style="background:${STATO_COLORS[m.stato]||'var(--muted)'};color:#fff;border-radius:5px;padding:2px 9px;font-size:10px;font-weight:600;">${m.stato?.replace('_',' ')||'—'}</span>
...
async function delMan(id){if(!confirm('Eliminare?'))return;await api('/api/manutenzioni/'+id,{method:'DELETE'});loadMan();toast('Eliminata','error');}
