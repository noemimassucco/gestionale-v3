...
// ═══════════════════════════════════════════════════════════
// TAB SWITCH (Attivi / Ex Clienti / Lead)
// ═══════════════════════════════════════════════════════════
let _clientiTabAttivo = 'attivo';

function switchClienteTab(stato, btn) {
  ...
  if (btn) {
    ...
    const auto = document.querySelector('.clienti-tab-btn[data-stato="' + stato + '"]');
    if (auto) {
      auto.classList.add('active');
      auto.style.color = 'var(--primary-dark)';
      auto.style.borderBottom = '2px solid var(--primary)';
      auto.style.fontWeight = '600';
    }
  }

  const tableWrap = document.getElementById('clienti-table-wrap');
  const cardsWrap = document.getElementById('lead-cards-wrap');
  const leadFiltri = document.getElementById('lead-filtri');

  if (stato === 'lead') {
    if (tableWrap)  tableWrap.style.display  = 'none';
    if (cardsWrap)  cardsWrap.style.display  = '';
    if (leadFiltri) leadFiltri.style.display = 'flex';
    loadLeadCards();
  } else {
    if (tableWrap)  tableWrap.style.display  = '';
    if (cardsWrap)  cardsWrap.style.display  = 'none';
    if (leadFiltri) leadFiltri.style.display = 'none';
    loadClienti(stato);
  }
}

async function loadClienti(stato) {
  stato = stato || _clientiTabAttivo || 'attivo';
  const tb = document.getElementById('tb-inquilini');
  if (!tb) return;
  tb.innerHTML = '<tr><td colspan="8" class="empty">Caricamento…</td></tr>';

  const data = await api('/api/clienti?stato=' + encodeURIComponent(stato));
  if (!data || data.error) {
    tb.innerHTML = '<tr><td colspan="8" class="empty">Errore caricamento clienti</td></tr>';
    return;
  }
  if (!data.length) {
    const msg = stato === 'attivo' ? 'Nessun cliente attivo' : 'Nessun ex cliente';
    tb.innerHTML = '<tr><td colspan="8" class="empty">' + msg + '</td></tr>';
    _updateClienteTabCounts();
    return;
  }

  tb.innerHTML = data.map(i => {
    const nome = i.ragione_sociale || [i.nome, i.cognome].filter(Boolean).join(' ') || '—';
    const stBg = stato === 'attivo' ? 'rgba(16,185,129,.15)' : 'rgba(100,116,139,.15)';
    const stCol = stato === 'attivo' ? 'var(--green)' : 'var(--muted)';
    const stLbl = stato === 'attivo' ? '🟢 Attivo' : '🔘 Ex';
    return '<tr>' +
      '<td><input type="checkbox" class="sel-check inquilini-chk" data-id="' + i.id + '" onchange="genToggle(\'inquilini\',' + i.id + ',this)"></td>' +
      '<td class="td-bold">' + esc(nome) + '</td>' +
      '<td class="td-mono" style="font-size:11px;">' + esc(i.piva || i.cf || '—') + '</td>' +
      '<td>' + esc(i.citta || '—') + '</td>' +
      '<td>' + esc(i.tel || '—') + '</td>' +
      '<td style="font-size:11px;">' + esc(i.email || '—') + '</td>' +
      '<td><span style="font-size:10px;padding:2px 7px;border-radius:10px;background:' + stBg + ';color:' + stCol + ';">' + stLbl + '</span></td>' +
      '<td><div style="display:flex;gap:4px;">' +
        '<button class="btn btn-edit btn-sm" onclick="openAnaById(\'inquilino\',' + i.id + ')">✏️</button>' +
        '<button class="btn btn-danger btn-sm" onclick="delAna(\'inquilini\',' + i.id + ')">✕</button>' +
      '</div></td>' +
    '</tr>';
  }).join('');
...
async function _updateClienteTabCounts() {
  try {
    ...
    let nAtt = 0, nEx = 0, nLead = 0;
    all.forEach(c => {
      const st = c.stato_calcolato || (c.is_lead ? 'lead' : 'attivo');
      if (st === 'attivo') nAtt++;
      else if (st === 'ex') nEx++;
      else if (st === 'lead') nLead++;
    });
    const setCnt = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
    setCnt('cnt-attivi', nAtt);
    setCnt('cnt-ex', nEx);
    setCnt('cnt-lead', nLead);
  } catch(e) {}
}
