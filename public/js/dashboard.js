...
async function loadDashboard(){
  ...
  if(sgEl)sgEl.innerHTML=(subs||[]).slice(0,8).map(s=>{
    ...
    return`<div class="home-sub-card" onclick="openSubDetail(${s.id})" style="cursor:pointer;">
      <div class="flex" style="margin-bottom:4px;">${sal} <strong style="color:#0f172a;font-size:13px;">${esc(s.codice)}</strong>${s.manutenzioni_aperte>0?`<span class="badge" style="background:rgba(184,134,11,.2);color:var(--accent-bg);margin-left:auto;">🔨${s.manutenzioni_aperte}</span>`:''}
      </div>
