...
function showSection(name){
  ...
  if(name==='dashboard')loadDashboard();
    ...
    // Focus chat input when section opens
...
function showSubCtx(e, subId, codice) {
  ...
  menu.innerHTML = `
    ...
    <div class="ctx-item" onclick="closeCtx();openModalInt();setTimeout(()=>{const el=document.getElementById('fi-sub');if(el)el.value=${subId};},100)">➕ Nuovo intervento</div>
    ...
    <div class="ctx-sep"></div>
    <div class="ctx-item" onclick="closeCtx();exportSubCSV(${subId},'${codice}')">⬇️ Esporta SUB</div>
    <div class="ctx-sep"></div>
    <div class="ctx-item danger" onclick="closeCtx();delAna('subs',${subId})">🗑 Elimina SUB</div>`;
  document.body.appendChild(menu);
  // Position near click
  const x = Math.min(e.clientX, window.innerWidth - 210);
  const y = Math.min(e.clientY + 6, window.innerHeight - menu.offsetHeight - 10);
