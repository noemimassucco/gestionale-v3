...
function renderTbSubs() {
  ...
  const tb2early = document.getElementById('tb-subs-ana');
  if (!DB.subs || !DB.subs.length) {
    const emptyHtml = '<tr><td colspan="10" class="empty">Nessun SUB. Aggiungili da "+ Nuovo SUB".</td></tr>';
    if (tb) tb.innerHTML = emptyHtml;
    if (tb2early) tb2early.innerHTML = emptyHtml;
    return;
  }
  const rowsHtml = DB.subs.map(s => {
