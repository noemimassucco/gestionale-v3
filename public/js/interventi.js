...
async function openDet(id){
  ...
  const allHtml=(inv.allegati||[]).length?`<div style="margin-top:12px;"><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:7px;">Allegati</div>${(inv.allegati||[]).map(a=>`<div class="all-item"><div class="all-icon">${a.tipo==='foto'?'📷':a.tipo==='fattura'?'🧾':'📄'}</div><span class="all-nome">${esc(a.nome)}</span><span class="all-dim">${a.dimensione?Math.round(a.dimensione/1024)+' KB':''}</span><a href="${fileUrl(a.url)}" target="_blank" style="font-size:11px;color:var(--accent);">Apri</a><button class="btn btn-danger" onclick="delAll(${a.id},${inv.id})">✕</button></div>`).join('')}</div>`:''
...
// ── populateSedeSelect — popola un select sede con DB.sedi ──────────
function populateSedeSelect(sedeSelectId) {
  const sel = document.getElementById(sedeSelectId);
  if (!sel) return;
  sel.innerHTML = '<option value="">— Tutte le sedi —</option>' +
    (DB.sedi || []).map(s => '<option value="' + s.id + '">' + esc(s.nome) + '</option>').join('');
}
