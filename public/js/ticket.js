...
async function loadTicket(){
  ...
  const data=await api('/api/ticket?'+p);if(!data)return;
  ...
  el.innerHTML=data.map(t=>`
    <div class="tick-card ${t.stato||'aperto'}" data-id="${t.id}">
      <div class="int-card-hdr">
        ...
        ${PRIOR_ICONS2[t.priorita]||'⚪'} <strong style="font-size:13px;color:#fff;">${esc(t.titolo)}</strong>
        ...
        <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:${t.stato==='chiuso'?'rgba(16,185,129,.2)':t.stato==='in_corso'?'rgba(245,158,11,.2)':'rgba(37,99,235,.2)'};color:${t.stato==='chiuso'?'var(--green)':t.stato==='in_corso'?'var(--gold)':'var(--accent)'};">${t.stato?.replace('_',' ')||'—'}</span>
...
async function delTicket(id){if(!confirm('Eliminare?'))return;await api('/api/ticket/'+id,{method:'DELETE'});loadTicket();toast('Eliminato','error');}
