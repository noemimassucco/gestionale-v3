...
async function sendChatPage(){
  ...
  if(r.dati?.length){
    html+=r.dati.slice(0,6).map(d=>{
      ...
      if(r.tipo==='fornitori')return`<div class="chat-result">🔧 <strong>${esc(d.ragione_sociale)}</strong> — ${d.num_int||0} int. · <strong style="color:var(--gold2);">€ ${parseFloat(d.totale||0).toLocaleString('it-IT')}</strong></div>`;
