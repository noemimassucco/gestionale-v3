...
async function loadBollette(){
  ...
  const data=await api('/api/bollette?'+p);if(!data)return;
...
async function delBolletta(id){if(!confirm('Eliminare?'))return;await api('/api/bollette/'+id,{method:'DELETE'});loadBollette();toast('Eliminata','error');}
