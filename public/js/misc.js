...
function openZuc(type){zucType=type;zucRows=[];zucMap={};document.getElementById('zuc-ttl').textContent='Import '+(type==='fornitori'?'Fornitori':'Inquilini')+' da Zucchetti';document.getElementById('zuc-s1').style.display='';document.getElementById('zuc-s2').style.display='none';document.getElementById('zuc-ok').style.display='none';document.getElementById('zucFile').value='';document.getElementById('modal-zuc').classList.add('open');}
  ...
  if (tab === 'sub') {
    ...
    el.innerHTML = Object.entries(bySede).map(([sede, subs]) => {
      ...
      return `<div style="margin-bottom:20px;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 16px;background:linear-gradient(90deg,rgba(37,99,235,.2),rgba(37,99,235,.05));border:1px solid rgba(37,99,235,.3);border-radius:10px 10px 0 0;">
          <span style="font-family:'Playfair Display',serif;font-size:16px;color:#fff;">📍 ${esc(sede)}</span>
            ...
            <div class="card" style="margin-bottom:8px;cursor:pointer;" onclick="document.getElementById('ff-sub').value='${d.sub_id}';showSec('interventi',document.querySelectorAll('.nb')[0]);loadInt();">
              <div class="flex-between" style="margin-bottom:8px;">
                <div class="flex">
                  ...
                  <span style="font-family:'Playfair Display',serif;font-size:15px;color:#fff;font-weight:700;">SUB ${esc(d.sub||'N/D')}</span>
    ...
    el.innerHTML = data.anni.map(a => {
      ...
      return `<div class="card" style="margin-bottom:12px;">
        <div class="flex-between" style="margin-bottom:12px;">
          <div class="flex">
            <span style="font-family:'Playfair Display',serif;font-size:20px;color:#fff;font-weight:700;">${a.anno}</span>
    ...
    el.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;">` +
      data.map(s=>`<div class="card">
        <div style="font-family:'Playfair Display',serif;font-size:18px;color:#fff;margin-bottom:12px;">📍 ${esc(s.sede||'—')}</div>
...
async function renderSubDetTab(tab) {
  ...
  if(tab==='identita'){    let istatAlert='';
    ...
    el.innerHTML=`<div class="flex-between" style="margin-bottom:12px;"><span style="font-size:13px;color:var(--muted);">${items.length} manutenzioni</span><button class="btn btn-primary btn-sm" onclick="closeM('modal-sub-det');openModalMan();">+ Nuova</button></div>
      ${items.length?items.map(m=>`<div class="int-card" style="border-left:3px solid ${pc[m.priorita]||'var(--border)'};margin-bottom:8px;"><div class="int-card-hdr">${pi[m.priorita]||'⚪'} <strong style="font-size:13px;color:#fff;">${esc(m.tipo)}</strong><span style="font-size:11px;padding:2px 8px;border-radius:5px;background:var(--surface2);color:var(--muted);">${m.stato||'—'}</span>${m.costo?`<span class="td-price" style="margin-left:auto;">€ ${parseFloat(m.costo).toLocaleString('it-IT')}</span>`:''}</div><div style="font-size:11px;color:var(--muted);margin-top:4px;">${m.prossima_scadenza?'Prossima: '+fmt(m.prossima_scadenza)+' · ':''}${esc(m.ricorrenza||'Una tantum')}${m.fornitore_nome?' · '+esc(m.fornitore_nome):''}</div></div>`).join(''):'<div class="empty">Nessuna manutenzione.</div>'}`;
    ...
    el.innerHTML=`
      <div style="text-align:center;padding:20px 0;">
            ...
            ${gen.padri.map(p=>`
              <div style="text-align:center;">
                <div class="tree-node" onclick="openSubDetail(${p.sub_padre})" style="flex-direction:column;padding:12px 16px;gap:4px;">
                  ...
                  <strong style="color:#fff;">${esc(p.codice_padre||'—')}</strong>
        ...
        <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(37,99,235,.15);border:2px solid var(--accent);border-radius:10px;padding:14px 20px;margin:8px 0;">
          ...
          <div>
            <div style="font-size:16px;font-weight:700;color:#fff;">SUB ${esc(s?.codice||'—')}</div>
          ...
          <div style="display:flex;justify-content:center;gap:10px;flex-wrap:wrap;">
            ${gen.figli.map(f=>`
              <div class="tree-node" onclick="openSubDetail(${f.sub_figlio})" style="flex-direction:column;padding:12px 16px;gap:4px;">
                ...
                <strong style="color:#fff;">${esc(f.codice_figlio||'—')}</strong>
  ...
  return `
    ...
    ${anni.map(anno => `
      <div style="margin-bottom:14px;">
        <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:8px;display:flex;align-items:center;gap:8px;">
          📅 ${anno}
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:7px;">
          ${pagPerAnno[anno].map(p=>`
            <div onclick="deletePagamento(${p.id})" title="Clicca per eliminare" style="background:var(--surface2);border:1px solid ${p.stato==='pagato'?'rgba(16,185,129,.3)':p.stato==='insoluto'?'rgba(239,68,68,.3)':'var(--border)'};border-radius:8px;padding:9px 11px;cursor:pointer;transition:all .2s;" onmouseover="this.style.opacity='.7'" onmouseout="this.style.opacity='1'">
              <div style="font-size:11px;font-weight:600;color:#fff;">${mesi[p.mese]}</div>
...
function renderTabInquilini(data) {
  ...
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
      <div>
        <div style="font-size:13px;font-weight:600;color:#fff;">Inquilino attuale: ${esc(s.inquilino_nome||'Libero')}</div>
    ...
    ${!storico.length ? '<div class="empty">Nessuno storico inquilini registrato.</div>' :
      storico.map(si => `
        <div style="display:flex;align-items:flex-start;gap:12px;padding:12px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;margin-bottom:8px;">
          ...
          <div style="flex:1;">
            <div style="font-size:13px;font-weight:600;color:#fff;">${esc(si.inquilino_nome||'—')}</div>
