...
async function renderSubDetTab(tab) {
  ...
  if(tab==='identita'){    let istatAlert='';
    ...
    el.innerHTML=`<div class="flex-between" style="margin-bottom:12px;"><span style="font-size:13px;color:var(--muted);">${items.length} documenti</span><button class="btn btn-primary btn-sm" onclick="closeM('modal-sub-det');openModalDoc();">+ Nuovo documento</button></div>
      ${items.length?items.map(d=>`<div class="doc-card"><div class="doc-icon">${icons[d.tipo]||'📂'}</div><div class="doc-info"><div class="doc-nome">${esc(d.nome)}</div><div class="doc-meta">${d.tipo}${d.data_documento?' · '+fmt(d.data_documento):''}${d.importo?' · € '+parseFloat(d.importo).toLocaleString('it-IT'):''}${d.fornitore_nome?' · '+esc(d.fornitore_nome):''}</div></div>${d.scadenza?`<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(184,134,11,.2);color:#fcd34d;">Scade ${fmt(d.scadenza)}</span>`:''}${d.url?`<a href="${d.url}" target="_blank" class="btn btn-edit btn-sm">👁</a>`:''}</div>`).join(''):'<div class="empty">Nessun documento.</div>'}`;
...
async function sendChat(){
  ...
  if(r.dati?.length){
    html+=r.dati.slice(0,5).map(d=>{
      ...
      if(r.tipo==='fornitori')return`<div class="chat-result">🔧 <strong>${esc(d.ragione_sociale)}</strong> — ${d.num_int} int · <strong style="color:var(--accent-bg);">€ ${parseFloat(d.totale||0).toLocaleString('it-IT')}</strong></div>`;
      if(r.tipo==='subs')return`<div class="chat-result" onclick="document.getElementById('ff-sub').value='${d.id||''}';showSec('interventi',document.querySelectorAll('.nb')[0]);loadInt();">🏠 <strong>SUB ${esc(d.sub||d.codice||'?')}</strong> (${esc(d.sede||'')}) — <strong style="color:var(--accent-bg);">€ ${parseFloat(d.totale||0).toLocaleString('it-IT')}</strong></div>`;
