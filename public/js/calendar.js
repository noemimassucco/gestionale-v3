...
async function loadCalendario(){
  ...
  const typeColors={documento:'rgba(124,58,237,.2)',manutenzione:'rgba(239,68,68,.2)',bolletta:'rgba(14,165,194,.2)',contratto_istat:'rgba(163,230,53,.2)'};
  el.innerHTML=Object.entries(byDate).map(([date,events])=>`
    <div style="margin-bottom:16px;">
      <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:1.5px;padding:0 0 7px;margin-bottom:8px;border-bottom:1px solid var(--border);">
        📅 ${date==='senza data'?date:fmt(date)} ${date!=='senza data'?`<span style="font-size:10px;background:${getDaysUntil(date)<0?'rgba(239,68,68,.2)':getDaysUntil(date)<7?'rgba(245,158,11,.2)':'rgba(100,116,139,.1)'};color:${getDaysUntil(date)<0?'var(--red)':getDaysUntil(date)<7?'var(--gold)':'var(--muted)'};padding:1px 7px;border-radius:8px;margin-left:6px;">${getDaysUntil(date)===0?'Oggi':getDaysUntil(date)<0?`Scaduto ${-getDaysUntil(date)}gg fa`:`${getDaysUntil(date)}gg`}</span>`:''}
      </div>
      ${events.map(ev=>`<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--surface);border:1px solid var(--border);border-left:3px solid ${typeColors[ev.tipo]||'var(--border)'};border-radius:8px;margin-bottom:7px;">
        ...
        <div style="flex:1;">
          <div style="font-size:13px;font-weight:600;color:#fff;">${esc(ev.titolo||'—')}</div>
