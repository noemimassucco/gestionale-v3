...
function openAna(type,obj=null){
  ...
  const html={
    sub:`
      ...
      <div style="grid-column:1/-1;" id="istat-preview-wrap" class="hidden">
        <div style="background:rgba(184,134,11,.08);border:1px solid rgba(184,134,11,.25);border-radius:8px;padding:12px 14px;">
          <div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:6px;">📈 ISTAT calcolato automaticamente</div>
  ...
  if(obj){
    ...
    else if(type==='categoria'){s('a-nome').value=obj.nome||'';s('a-ico').value=obj.icona||'';s('a-col').value=obj.colore||'#6b8e6b';}
  }
...
async function saveAna(){
  ...
  loadDD().then(() => {
    if (anaType==='sub') renderTbSubs();
    else if (anaType==='fornitore') renderTbForn();
...
async function delAna(table, id) {
  ...
  if (DB[table])     DB[table]     = DB[table].filter(x => Number(x.id) !== Number(id));

  // Re-render immediatamente
  if (table==='subs') renderTbSubs();
  else if (table==='fornitori') renderTbForn();
  else if (table==='inquilini') renderTbInq();
  else if (table==='sedi') renderTbSedi();
  else if (table==='categorie') renderTbCat();

  toast('🗑 ' + (labels[table]||table) + ' eliminato');

  // API in background
  const r = await api('/api/' + table + '/' + id, { method: 'DELETE' });
  if (r?.error) { toast('⚠️ Errore server: ' + r.error, 'error'); }
  loadDD(); // sincronizza con DB reale
}
