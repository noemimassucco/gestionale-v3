...
app.put('/api/subs/:id', authMiddleware, async (req, res) => {
  const { codice, ex_sub, sede_id, piano, inquilino_id, note } = req.body;
  // Leggi stato attuale per storico
  ...
  // Registra storia se cambiano dati importanti
  if (old.rows.length) {
    const o = old.rows[0];
    const cambiamenti = [];
    if (o.codice !== codice) cambiamenti.push(`Codice: ${o.codice} → ${codice}`);
    if ((o.ex_sub||'') !== (ex_sub||'')) cambiamenti.push(`Ex SUB: ${o.ex_sub||'—'} → ${ex_sub||'—'}`);
    if (o.inquilino_id !== (parseInt(inquilino_id)||null)) cambiamenti.push(`Inquilino cambiato`);
    if (o.sede_id !== (parseInt(sede_id)||null)) cambiamenti.push(`Sede cambiata`);
    if (cambiamenti.length) {
      await pool.query(
        'INSERT INTO sub_storia (sub_id,tipo,titolo,descrizione,dati_vecchi,dati_nuovi,created_by) VALUES ($1,$2,$3,$4,$5,$6,$7)',
        [req.params.id,'modifica','Modifica anagrafica SUB',cambiamenti.join(' | '),JSON.stringify(o),JSON.stringify(req.body),req.user.id]
      );
    }
  }
  res.json(r.rows[0]);
